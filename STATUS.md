...
**Veredito:** GEO_MATCH_CENTROIDE_BAIXA_CONFIANCA_PUBLICADO_OK / SPEC010_BACKEND_PUBLICADO_OK / PERFIL_ADVOGADO_SOCIAIS_PRODUCAO_OK / MIGRATION_0006_APLICADA_OK / MIGRATION_0005_APLICADA_OK / ADMIN_OPERACIONAL_ORACOES_USUARIOS_MIDIA_PRODUCAO_OK / MIGRATION_0004_APLICADA_OK / CLIENT_SIGNUP_PRODUCAO_OK / CLIENT_SIGNUP_BACKEND_LOCAL_OK / MATCH_EVENTO_NAO_BLOQUEIA_RESPOSTA_LOCAL_OK / SPEC008_PARTE3_RETENCAO_ORACAO_PUBLICADA_OK

- [x] Primeiro acesso do advogado implementado localmente em 2026-06-05: `POST /v1/admin/lawyers` provisiona convite Supabase Auth por e-mail antes de criar `profiles.role=lawyer` com `id=auth.users.id`; `POST /v1/admin/lawyers/:id/access-invite` ativa legados; `POST /v1/auth/change-password` atualiza senha autenticada e limpa `mustChangePassword`; `GET /v1/me` retorna estado seguro de primeiro acesso. Migration `0007_lawyer_access_invites.sql` adiciona campos operacionais e RPC de revinculo legado nao destrutivo. Gate local: `npm run harness` exit 0 (69 testes, build, migration dry-run e smoke). Pendente para producao: aplicar migration 0007 e publicar Railway.
- [x] Validacao GEO/match publicada em 2026-06-05 no commit `e31981b`: `POST /v1/match` usa lat/lng do cliente e PostGIS; eventos recentes de producao com `distanceKm=27.9` foram associados a advogados de Brasilia/DF cuja coordenada persistida veio de fallback amplo do provider (`cep_centroid` com baixa confianca). Backend agora bloqueia `cep_centroid + confidence=low` como coordenada elegivel para match/aprovacao e deixa geocode admin como recuperavel/coordenada pendente. Amanda Silva e Israel Gomes foram corrigidos na base de producao para a referencia de bairro de Samambaia; aprovados afetados ficaram com offset `0 km` contra a referencia. Gates: `npx vitest run tests/geocoding.test.ts tests/app.test.ts` exit 0, `npm run typecheck` exit 0, `npm run harness` exit 0, smoke especifico Railway OK e `prod:smoke` Railway exit 0.
