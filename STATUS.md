...
- Bugfix local 2026-06-04: aprovacao de advogado legado sem coordenada agora tenta re-geocodificar automaticamente usando o CEP ja salvo antes de bloquear. Causa confirmada em producao: `PATCH /v1/admin/lawyers/:id` com apenas `{ "status": "approved" }` retornava `422` quando `officeLat/officeLng` estavam nulos. Backend `npm run harness` exit 0 com 64 testes.
